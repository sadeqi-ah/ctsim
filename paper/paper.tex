...
% Placeholder for numbers that must come from the simulator run.
% Grep for \TBD before submission -- none may remain.
...
coupling and turns latency into a spendable design variable. Results hold
across five topologies with diameters from 1 to 26, confirming that the
effect is architectural rather than topological.
\end{abstract}
...
In the calibration step, the two free parameters of the physical
abstraction---the per-attempt loss rate and the slot duration
$\Tslot$---are fixed so that the simulator reproduces the published
measurements of BlueFlood on the corresponding configuration, namely a
packet delivery ratio of approximately 99.9\,\% at a radio duty cycle of
0.4\,\% \cite{alnahas19blueflood}, \cite{alnahas21blueflood}. The
parameters are thus derived from measured behaviour on real hardware
rather than chosen for convenience.

In the prediction step, those parameters are frozen and the simulator is
asked to reproduce two independent published results that played no part
in the calibration: agreement across 180 nodes in 475\,ms for A2/2PC
\cite{alnahas17a2}, and consensus across 188 nodes in 289\,ms for Wireless
Paxos \cite{poirot19paxos}. Because nothing is retuned between the two
steps, the outcome is a blind prediction rather than a fit, which is what
gives the exercise its evidential value. We adopt an acceptance criterion
of 10--20\,\% relative error, stated in advance so that it cannot be
chosen after the fact. The simulator reproduces the A2 configuration
within \TBD{A2 error} and the Wireless Paxos configuration within
\TBD{WPaxos error}. All three profiles are maintained as regression tests
in the simulator's test suite, so that the calibration cannot silently
...
difference is stable; we report the range over which it holds and
\TBD{any regime in which it does not}. The topology sweep of
...
This matters for the interpretation of the preceding figure. While some
...
The conventional expectation is that a better network benefits every
protocol. The measurements show that this holds for one family only.
Table~\ref{tab:diameter} summarises the change in throughput on moving
from the random topology to the full mesh, that is, on improving
connectivity by a factor of four in diameter.
...
The design consequence is direct. In deployments where network quality can
be improved---by raising transmission power, increasing node density or
improving routing---only the \CI{} architecture, together with
single-phase protocols, converts that investment into performance. For
Paxos and 2PC over \CE, investment in network infrastructure is largely
wasted.
...
26 in a network of 27 nodes, every node has just two neighbours and the
path between the two ends of the network passes through all of them. Every
...
loss and at scale the loss of decisions itself dominates, with the commit
rate dropping to 23.2\,\% in the largest scalability network and 0.2\,\%
for 2PC on the line topology. The cumulative progress view then shows that the
high latency of \CI{} is
...
Third, the results are stable across the topology space, and the boundary
case of TOM shows that even with no concurrency at all---a pipeline of
depth one---the duty-cycle saving of a radio that is allowed to sleep is
by itself sufficient to preserve the advantage of \CI{}.
