...
\centering
\resizebox{3.4in}{!}{%
\begin{tikzpicture}[x=5.7cm,y=1.25cm,font=\footnotesize]
  \draw[gray!35] (0,-1.75) grid[xstep=0.25,ystep=1] (1,1.75);
  \draw[->] (0,-1.75) -- (1.08,-1.75) node[anchor=west] {$t$};
  \draw[->] (0,-1.75) -- (0,1.90) node[anchor=south] {$y(t)$};
  \foreach \y in {-1,0,1} {\draw (-0.012,\y) -- (0.012,\y);}
  \node[anchor=east] at (-0.02,1) {$1$};
  \node[anchor=east] at (-0.02,-1) {$-1$};
  \node[anchor=north] at (0,-1.78) {$0$};
  \node[anchor=north] at (0.5,-1.78) {$0.5$};
  \node[anchor=north] at (1,-1.78) {$1$};
  \draw[blue!75!black, densely dotted, line width=0.7pt]
    plot[domain=0:1,samples=180,smooth]
      (\x,{0.72*sin(deg(2*pi*5.5*\x+pi/2))});
  \draw[orange!85!black, dashed, line width=0.7pt]
    plot[domain=0:1,samples=180,smooth]
      (\x,{0.72*sin(deg(2*pi*7.6*\x-pi/2))});
  \draw[purple!75!black, dashed, line width=0.8pt]
    plot[domain=0:1,samples=180,smooth]
      (\x,{1.44*cos(deg(pi*(5.5-7.6)*\x))});
  \draw[purple!75!black, dashed, line width=0.8pt]
    plot[domain=0:1,samples=180,smooth]
      (\x,{-1.44*cos(deg(pi*(5.5-7.6)*\x))});
  \draw[orange!70!black, line width=0.9pt]
    plot[domain=0:1,samples=240,smooth]
      (\x,{0.72*sin(deg(2*pi*5.5*\x+pi/2))
             +0.72*sin(deg(2*pi*7.6*\x-pi/2))});
  \node[anchor=west,fill=white,inner sep=1pt] at (0.03,1.70)
    {\textcolor{blue!75!black}{$y_1$}\, dotted\quad
     \textcolor{orange!85!black}{$y_2$}\, dashed\quad
     \textcolor{orange!70!black}{$y_1+y_2$}\quad
     \textcolor{purple!75!black}{envelope}};
\end{tikzpicture}}
\caption{Two concurrent sinusoids with different frequencies and phases,
their superposition, and its amplitude envelope. This is an original
vector redraw adapted from \cite{alnahas19blueflood}; it is not a
...
\begin{tikzpicture}[font=\scriptsize]
  \foreach \X/\Panel in {0/a,3.45/b,6.90/c} {
    \begin{scope}[shift={(\X,0)}]
      \fill[gray!22] (0,0) rectangle (3.0,2.15);
      \draw[->] (0,0) -- (3.18,0) node[anchor=west] {$\tau/T$};
      \draw[->] (0,0) -- (0,2.42) node[anchor=south] {$\varphi_c/\pi$};
      \draw[thin] (0,0) rectangle (3.0,2.15);
      \draw[red!75!black, line width=0.55pt]
        plot[domain=0:3,samples=80,smooth]
          (\x,{1.08+0.72*sin(deg(180*\x/3))});
      \draw[red!75!black, line width=0.55pt]
        plot[domain=0:3,samples=80,smooth]
          (\x,{1.08-0.72*sin(deg(180*\x/3))});
      \draw[red!75!black, line width=0.55pt]
        plot[domain=0:3,samples=80,smooth]
          (\x,{1.08+0.72*sin(deg(180*\x/3)+180)});
      \draw[red!75!black, line width=0.55pt]
        plot[domain=0:3,samples=80,smooth]
          (\x,{1.08-0.72*sin(deg(180*\x/3)+180)});
      \fill[black] (0.65,0.80) .. controls (1.10,0.30) and (1.90,0.30) .. (2.35,1.08)
        .. controls (1.90,1.86) and (1.10,1.86) .. (0.65,0.80) -- cycle;
      \draw[white, line width=0.6pt] (0.65,0.80) .. controls (1.10,0.30) and (1.90,0.30) .. (2.35,1.08)
        .. controls (1.90,1.86) and (1.10,1.86) .. (0.65,0.80) -- cycle;
      \node[white,align=center] at (1.50,1.08) {capture\\zone};
      \node[anchor=north] at (1.50,-0.28) {(\Panel)};
    \end{scope}
  }
  \node[anchor=south] at (1.50,2.55) {uncoded};
  \node[anchor=south] at (4.95,2.55) {coded, HDD};
  \node[anchor=south] at (8.40,2.55) {coded, SDD};
\end{tikzpicture}}
\caption{Conceptual capture zones in the time-offset/phase-offset plane.
The dark regions indicate parameter combinations in which one of two
concurrent transmissions is captured; the three panels summarize the
qualitative cases in the source study. This original vector schematic is
adapted from \cite{wilhelm14reception} and is not a reproduction of any
measurement heatmap.}
