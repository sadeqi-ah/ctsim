...
% over CI) are drawn inline with TikZ, so no external file is needed for
% them and nothing has to be placed in figures/ by hand.
\usepackage{tikz}
...
% No TBD or TODO control sequences may remain at submission.
...
total-order multicast (TOM) on a CI substrate, using a unified logical message format with a current-term participation
bitmap, cumulative commitment, and NACK-driven cascading recovery. The
bitmap is merged locally into the pending state of the current and earlier
terms, allowing several decisions to remain in flight while preserving
total order. Extensive slot-level simulation shows that the
pipeline sustains, on average, about 12 concurrent proposals for Paxos and
about 21 for 2PC, yielding 4.083x higher baseline throughput (global maximum 4.268x) and 5.672x better
baseline per-decision energy efficiency (global maximum 5.987x) than CE-based counterparts. We
...
control header carrying the current term, a cumulative commitment marker,
a current-term participation bitmap, and a NACK term. At receivers, the
bitmap is OR-merged into the local pending state of the current and
earlier terms. We use this format to implement three protocols over \CI: pipelined Paxos, non-blocking 2PC, and
...
every node observes the same deterministic sequence of
current-term participation markers and applies the same local merge rule,
2PC commits without a coordinator and therefore never blocks,
...
for Paxos and about 21 for 2PC, that this concurrency yields $4.083\times$
higher baseline throughput (global maximum $4.268\times$) and $5.672\times$ better baseline
per-decision energy (global maximum $5.987\times$), and---most importantly---that \CE{} architectures obey
the identity $\Ece = \Nnodes L$, which holds exactly and by construction
across 15 protocol-topology CE aggregates, each over 15 seeds (225 runs), spanning a $26\times$ range of network
...
\centering
\resizebox{3.4in}{!}{%
\begin{tikzpicture}[x=3.4cm,y=0.48cm,font=\footnotesize]

  % Axes and grid
  \draw[gray!30]
    (0,-2) grid[xstep=0.5,ystep=1] (1,2);
  ...
  % Vertical y-axis label, as in the reference
  \node[rotate=90] at (-0.22,0) {$y(t)$};
  ...
  \node[anchor=east] at (-0.035,2) {$2$};
  \node[anchor=east] at (-0.035,0) {$0$};
  \node[anchor=east] at (-0.035,-2) {$-2$};

  \node[anchor=north] at (0,-2.05) {$0$};
  \node[anchor=north] at (0.5,-2.05) {$0.5$};
  \node[anchor=north] at (1,-2.05) {$1$};

  % First sinusoid:
  % y_1 = sin(2 pi 5.5 t + 90 degrees)
  \draw[
    blue!75!black,
    densely dotted,
    line width=0.7pt
    ...
    plot[
      ...
      (\x,{sin(deg(2*pi*5.5*\x+pi/2))});

  % Second sinusoid:
  % y_2 = sin(2 pi 7.6 t - 90 degrees)
  \draw[
    red!75!black,
    dashed,
    line width=0.7pt
  ...
  \draw[
    ...
    plot[
      ...
      (\x,{
        sin(deg(2*pi*5.5*\x+pi/2))
        + sin(deg(2*pi*7.6*\x-pi/2))
      });

  % Positive amplitude envelope:
  % 2 |sin(pi (7.6 - 5.5) t)|
  \draw[
    purple!75!black,
    dashed,
...
\caption{Two concurrent sinusoids with different frequencies and phases,
their superposition, and its positive amplitude envelope. This is an
original vector redraw adapted from \cite{alnahas19blueflood}; it is not a
reproduction of the source raster.}
\label{fig:ct-beating}
\end{figure}

The physical-layer effect is easiest to see before the protocol detail.
Fig.~\ref{fig:ct-beating} places the two carrier traces on one common
axis: their sum oscillates rapidly inside a slower envelope, illustrating
how concurrent transmissions can reinforce or cancel at the receiver.
...
\begin{figure}[!t]
\centering
\includegraphics[width=\linewidth]{figures/ct_capture_fig8_vector.pdf}
\caption{Relation between error rates and signal parameters, time offset
$\tau$ and carrier phase offset $\varphi_c$ (with SIR $=-40$ dB).
An unsynchronized packet is successfully received if the parameter
combinations fall inside the dark capture zones.}
\label{fig:ct-capture}
\end{figure}

Fig.~\ref{fig:ct-capture} shows the complementary case. When the two
overlapping transmissions are not identical, the receiver locks onto
whichever arrives with a sufficient power margin and suppresses the other;
the dark regions mark the qualitative capture windows rather than a new
measurement made by our simulator.
...
\cite{trobinger24uwbcontrol}. Recent work has improved the reliability of
synchronous-transmission reception itself by voting on individual bits
\cite{rangwala24stxvote}; that line of work is complementary to, and
orthogonal to, the concern of this paper, whose contribution lies in the
ordering and concurrency of agreement instances rather than in raw packet
reception. This literature has sharpened the picture
...
overlap simply does not arise. A recent preprint extends the same
data-moving use of constructive interference to scalable,
energy-efficient predictive data collection in wireless sensor networks
\cite{muldoon25stair}, and the same observation applies to it. To the
best of our knowledge, pipelining
...
\subsubsection{Slots}
Time is divided into discrete slots. A simulator slot is a dimensionless
scheduling unit, not a calibrated physical duration: the simulator itself
contains no wall-clock duration. A physical implementation must simply
make each slot long enough to transmit or receive one radio packet and
perform the associated local processing \cite{ferrari11glossy}. In every
slot a node is in exactly one of three states: transmitting (Tx), receiving
(Rx), or asleep with the radio powered down. The third state is what makes
energy a
...
\begin{tikzpicture}[
  font=\footnotesize,
  rnd/.style={draw, minimum width=1.05cm, minimum height=0.42cm,
              inner sep=1pt},
  ph/.style={draw, fill=black!8, minimum width=1.05cm,
             minimum height=0.38cm, inner sep=1pt, font=\scriptsize},
]
\node[anchor=east, font=\scriptsize] at (-0.68,0.00) {flood rounds};
\node[rnd] at (0.00,0.00) {$R_{1}$};
\node[rnd] at (1.25,0.00) {$R_{2}$};
\node[rnd] at (2.50,0.00) {$R_{3}$};
\node[rnd] at (3.75,0.00) {$R_{4}$};
\node[rnd] at (5.00,0.00) {$R_{5}$};
\node[anchor=east, font=\scriptsize] at (-0.68,-0.80) {instance $k-1$};
\node[ph] at (0.00,-0.80) {prop.};
\node[ph] at (1.25,-0.80) {vote};
\node[ph] at (2.50,-0.80) {commit};
\node[anchor=east, font=\scriptsize] at (-0.68,-1.55) {instance $k$};
\node[ph] at (1.25,-1.55) {prop.};
\node[ph] at (2.50,-1.55) {vote};
\node[ph] at (3.75,-1.55) {commit};
\node[anchor=east, font=\scriptsize] at (-0.68,-2.30) {instance $k+1$};
\node[ph] at (2.50,-2.30) {prop.};
\node[ph] at (3.75,-2.30) {vote};
\node[ph] at (5.00,-2.30) {commit};
\draw[dashed] (1.90,-2.62) rectangle (3.10,0.28);
\node[anchor=north, font=\scriptsize, align=center] at (2.50,-2.70)
  {one round advances\\several instances};
\end{tikzpicture}}
...
round trip to close. Fig.~\ref{fig:pipeline-overlap} sketches that overlap on a
chain of consecutive flood rounds.
...
{last accepted\\term\\[-1pt]\scriptsize(optional)};
...
{current-term\\participation bitmap\\[-1pt]
\scriptsize($N$ logical bits)};
...
{NACK term\\[-1pt]\scriptsize(0 = none)};
...
{piggyback\\term\\[-1pt]\scriptsize(optional)};
...
\caption{Logical Paxos-over-CI packet format used by the simulator. Each
flood carries one bitmap for the current term. At every receiver, the
bitmap is merged into the local pending state of that term and all earlier
terms, which provides cumulative vote aggregation across successive flood
rounds. The payload carries the current proposal and optional recovery data
for a missing term.}
\label{fig:frame}
\end{figure}

Low-power radios impose tight frame budgets, so pipelining and state
recovery must share one compact logical packet structure rather than each
introducing its own control traffic. The packet has two parts: a control
header that identifies the current term and carries the current-term
participation bitmap, cumulative commitment marker, and NACK term, followed
by a payload carrying the current proposal and optional recovery data. Only
one bitmap is transmitted in each flood. At the receiver, that bitmap is
OR-merged into the local pending state of the current term and all earlier
terms, so cumulative aggregation occurs in local protocol state rather than
as a multi-term bitmap inside one packet. Fig.~\ref{fig:frame} shows this
...
\item \textbf{Current-term participation bitmap.}
The packet carries one $N$-bit logical bitmap for the term named in
\texttt{current\_term}. Bit $i$ indicates that node $i$ has participated in,
and voted affirmatively for, that term in the sender's local view. During
proposal-generation rounds, the bitmap is normally one-hot because each
round has one scheduled initiator. When a node receives the bitmap, it
OR-merges the set bits into the local pending state of the current term and
all earlier pending terms. Thus, only one bitmap is transmitted in each
flood, while older terms accumulate participation information across later
rounds. Each in-flight term maintains its own bitmap in the node's local
pending state.

\item \textbf{NACK term.}
The field is zero during normal operation. If a node detects a missing
term, it places the identifier of that term in \texttt{nack\_term}. A node
that already stores the requested term may return its proposal data through
the optional piggyback recovery fields in a later flood.
\end{itemize}

\subsubsection{Pipelined payload}
The payload has three logical components. The \texttt{proposal\_data} field
carries the proposal associated with \texttt{current\_term}. The optional
\texttt{piggyback\_term} field identifies a missing term requested by a
NACK. The \texttt{piggyback\_data} field carries the corresponding recovery
data. These recovery fields allow a lagging node to catch up without
interrupting the pipeline, because recovery data travels in the same floods
that carry new proposals forward.

The payload does not contain a separate bitmap for every in-flight term.
Each node stores those per-term bitmaps locally in its pending state, and
the current-term bitmap received in a flood is merged into the current and
earlier pending terms.

\subsubsection{Frame size and scalability}
The logical participation bitmap contains one bit per node, so its logical
size is $N$ bits. In the current simulator, the bitmap is represented as a
\texttt{Vec<bool>} inside the protocol state and is carried by the internal
packet serializer. The current serializer does not implement a bit-packed
wire representation. A future bit-packed implementation would require
$\lceil N/8\rceil$ bytes for this field. Therefore, the bitmap-size
statement in this paper refers to the logical protocol format, not to the
literal byte length of the simulator's JSON representation.
...
% ------------------------------------------------------------------
% Centered three-node communication topology
% ------------------------------------------------------------------
...
(4.55,0.6) -- (5.38,1) -- (6.21,0.6) -- cycle;
...
\Pcell{0.72cm}{black!12}{0.36}{0}{Round}
\Pcell{0.82cm}{black!12}{1.20}{0}{Init.}
\Pcell{1.32cm}{orange!18}{2.45}{0}{New term}
\Pcell{1.18cm}{black!12}{3.85}{0}{$T_1$}
\Pcell{1.18cm}{black!12}{5.15}{0}{$T_2$}
\Pcell{1.18cm}{black!12}{6.45}{0}{$T_3$}
\Pcell{1.18cm}{black!12}{7.75}{0}{$T_4$}
\Pcell{1.18cm}{black!12}{9.05}{0}{$T_5$}
\Pcell{0.92cm}{green!15}{10.30}{0}{last}

% ------------------------------------------------------------------
% Round 1: A opens T_1.
% ------------------------------------------------------------------

\Pcell{0.72cm}{black!3}{0.36}{-0.62}{$R_1$}
\Pcell{0.82cm}{orange!18}{1.20}{-0.62}{A}
\Pcell{1.32cm}{orange!18}{2.45}{-0.62}{$T_1:v_A$}
\Pcell{1.18cm}{blue!7}{3.85}{-0.62}{\texttt{A--}}
\Pcell{1.18cm}{black!3}{5.15}{-0.62}{--}
\Pcell{1.18cm}{black!3}{6.45}{-0.62}{--}
\Pcell{1.18cm}{black!3}{7.75}{-0.62}{--}
\Pcell{1.18cm}{black!3}{9.05}{-0.62}{--}
\Pcell{0.92cm}{green!5}{10.30}{-0.62}{0}

% ------------------------------------------------------------------
% Round 2: B opens T_2 and votes for T_1.
% ------------------------------------------------------------------

\Pcell{0.72cm}{black!3}{0.36}{-1.24}{$R_2$}
\Pcell{0.82cm}{orange!18}{1.20}{-1.24}{B}
\Pcell{1.32cm}{orange!18}{2.45}{-1.24}{$T_2:v_B$}
\Pcell{1.18cm}{blue!7}{3.85}{-1.24}{\texttt{AB--}}
\Pcell{1.18cm}{blue!7}{5.15}{-1.24}{\texttt{-B-}}
\Pcell{1.18cm}{black!3}{6.45}{-1.24}{--}
\Pcell{1.18cm}{black!3}{7.75}{-1.24}{--}
\Pcell{1.18cm}{black!3}{9.05}{-1.24}{--}
\Pcell{0.92cm}{green!5}{10.30}{-1.24}{0}

% ------------------------------------------------------------------
% Round 3: C opens T_3 and votes for T_1 and T_2.
% ------------------------------------------------------------------

\Pcell{0.72cm}{black!3}{0.36}{-1.86}{$R_3$}
\Pcell{0.82cm}{orange!18}{1.20}{-1.86}{C}
\Pcell{1.32cm}{orange!18}{2.45}{-1.86}{$T_3:v_C$}
\Pcell{1.18cm}{blue!7}{3.85}{-1.86}{\texttt{ABC}}
\Pcell{1.18cm}{blue!7}{5.15}{-1.86}{\texttt{-BC}}
\Pcell{1.18cm}{blue!7}{6.45}{-1.86}{\texttt{--C}}
\Pcell{1.18cm}{black!3}{7.75}{-1.86}{--}
\Pcell{1.18cm}{black!3}{9.05}{-1.86}{--}
\Pcell{0.92cm}{green!5}{10.30}{-1.86}{0}

% ------------------------------------------------------------------
% Round 4: A commits T_1 and opens T_4.
% ------------------------------------------------------------------

\Pcell{0.72cm}{black!3}{0.36}{-2.48}{$R_4$}
\Pcell{0.82cm}{orange!18}{1.20}{-2.48}{A}
\Pcell{1.32cm}{orange!18}{2.45}{-2.48}{$T_4:v_A$}
\Pcell{1.18cm}{green!22}{3.85}{-2.48}
{\texttt{ABC}\\[-1pt]\scriptsize quorum}
\Pcell{1.18cm}{blue!7}{5.15}{-2.48}{\texttt{ABC}}
\Pcell{1.18cm}{blue!7}{6.45}{-2.48}{\texttt{A-C}}
\Pcell{1.18cm}{blue!7}{7.75}{-2.48}{\texttt{A--}}
\Pcell{1.18cm}{black!3}{9.05}{-2.48}{--}
\Pcell{0.92cm}{green!22}{10.30}{-2.48}{1}

% ------------------------------------------------------------------
% Round 5: B commits T_2 and opens T_5.
% ------------------------------------------------------------------

\Pcell{0.72cm}{black!3}{0.36}{-3.10}{$R_5$}
\Pcell{0.82cm}{orange!18}{1.20}{-3.10}{B}
\Pcell{1.32cm}{orange!18}{2.45}{-3.10}{$T_5:v_B$}
\Pcell{1.18cm}{green!22}{3.85}{-3.10}{\texttt{ABC}}
\Pcell{1.18cm}{green!22}{5.15}{-3.10}
{\texttt{ABC}\\[-1pt]\scriptsize quorum}
\Pcell{1.18cm}{blue!7}{6.45}{-3.10}{\texttt{ABC}}
\Pcell{1.18cm}{blue!7}{7.75}{-3.10}{\texttt{AB--}}
\Pcell{1.18cm}{blue!7}{9.05}{-3.10}{\texttt{-B-}}
\Pcell{0.92cm}{green!22}{10.30}{-3.10}{2}

% ------------------------------------------------------------------
% Round 6: C commits T_3 and opens T_6.
% ------------------------------------------------------------------

\Pcell{0.72cm}{black!3}{0.36}{-3.72}{$R_6$}
\Pcell{0.82cm}{orange!18}{1.20}{-3.72}{C}
\Pcell{1.32cm}{orange!18}{2.45}{-3.72}{$T_6:v_C$}
\Pcell{1.18cm}{green!22}{3.85}{-3.72}{\texttt{ABC}}
\Pcell{1.18cm}{green!22}{5.15}{-3.72}{\texttt{ABC}}
\Pcell{1.18cm}{green!22}{6.45}{-3.72}
{\texttt{ABC}\\[-1pt]\scriptsize quorum}
\Pcell{1.18cm}{blue!7}{7.75}{-3.72}{\texttt{ABC}}
\Pcell{1.18cm}{blue!7}{9.05}{-3.72}{\texttt{-BC}}
\Pcell{0.92cm}{green!22}{10.30}{-3.72}{3}

\end{tikzpicture}%
}
\caption{Worked six-round trace of pipelined Paxos over constructive
interference on the three-node communication graph shown above. The
initiator order is $A\rightarrow B\rightarrow C$, the majority quorum is
$q=2$, and all nodes vote affirmatively. Each round initiates a new term
while the participation bitmaps of earlier terms advance concurrently.
A term is committed when its initiator observes a majority quorum. Each
CI flood contains one byte-identical frame; pipelining occurs across
successive flood rounds. The table shows the local pending-state view after
each round; each flood itself carries only one bitmap for its current term.
Older terms accumulate participation information because each received
current-term bitmap is merged into the local state of the current and
earlier terms.}
\label{fig:paxos}
\end{figure*}
% \begin{figure}[!t]
...
% \begin{tikzpicture}[
%   font=\scriptsize,
%   turn/.style={draw, minimum width=1.35cm, minimum height=0.44cm,
%                inner sep=1pt},
%   st/.style={font=\ttfamily\scriptsize},
% ]
% \node[turn] (t1) at (0.00,0) {turn of $n_1$};
% \node[turn] (t2) at (1.95,0) {turn of $n_2$};
% \node[turn] (t3) at (3.90,0) {turn of $n_3$};
% \node[turn] (t4) at (5.85,0) {turn of $n_4$};
% \draw[-{Latex[length=1.5mm]}] (t1) -- (t2);
% \draw[-{Latex[length=1.5mm]}] (t2) -- (t3);
% \draw[-{Latex[length=1.5mm]}] (t3) -- (t4);
% \node[st, below=2mm of t1] {1000};
% \node[st, below=2mm of t2] {1100};
% \node[st, below=2mm of t3] {1110};
% \node[st, below=2mm of t4] {1111};
% \node[anchor=north, align=center, below=7mm of t1]
%   {proposal for\\term $k$};
% \node[anchor=north, align=center, below=7mm of t2]
%   {bitmap grows\\by overhearing};
% \node[anchor=north, align=center, below=7mm of t3]
%   {majority quorum\\reached};
% \node[anchor=north, align=center, below=7mm of t4]
%   {last accepted\\term $:=k$};
% \node[anchor=south, font=\scriptsize] at (2.925,0.40)
%   {participation bitmap, example with four nodes};
% \end{tikzpicture}}
...
pipelined cycle in four steps, illustrated in Fig.~\ref{fig:paxos}.
...
the deterministic sequence of flood rounds. Each flood carries one
current-term participation bitmap. A node that receives the frame records
the bitmap in its local pending state and sets its own participation bit
for the current and earlier pending terms. When the initiator's turn
returns, the node checks the relevant local bitmap for a majority quorum.
Consequently, the packet does not carry separate bitmaps for all in-flight
terms; cumulative aggregation is performed by OR-merging each received
current-term bitmap into the local pending state of that term and all
earlier terms.
...
commit, sets its bit in its local copy of the participation bitmap and
floods that bit when its own turn arrives. The volatile, poorly
...
Thanks to overhearing, every node reconstructs the same accumulated
bitmap from the sequence of floods. The decision is consequently not taken
by the coordinator: once exactly one cycle has elapsed since the
transaction opened, \emph{each node} independently evaluates the bitmap.
Unlike Paxos, a node commits only if
...
evaluation is the same reconstructed bitmap and the instant of evaluation is
...
(mean $\Diam = 4.20$, range 4--5), scale-free (mean $\Diam = 4.13$, range 4--5) and line ($\Diam = 26$). Random
and scale-free are approximately matched in diameter (Table~\ref{tab:topologies}) but
...
(mean $\Diam = 4.20$) and 5\,\% link loss.
...
satisfied in the maintained bitmap. These are necessary conditions, and we
...
This conversion is a lower bound only. The identity
$\Tslot = T_{\mathrm{air}} + T_{\mathrm{IFS}} + 2\,T_{\mathrm{guard}}$
accounts for packet airtime, one inter-frame space, and the two guard
intervals. It excludes MCU wake-up, radio ramp-up, local packet processing,
and turnaround beyond $T_{\mathrm{IFS}}$; it also does not by itself account
for the multiple physical slots consumed by repeated transmissions within
a flood. Published
nominal firmware slot lengths are therefore not interchangeable with this
lower-bound conversion. Realised firmware slot lengths are approximately
2.34\,\% below nominal because of 32.768\,kHz timer quantisation, which
introduces a systematic bias in any comparison with hardware timing.

The prediction step freezes that parameter and tests two published results
that played no part in calibration. The mechanism is the slot-duration
model of Section~\ref{sec:method}: converting the simulated slot counts
into milliseconds with the bracketed $\Tslot$ and comparing against the
published testbed figures. At the pre-registered
$T_{\mathrm{guard}}=10\,\mu\mathrm{s}$ acceptance point, with relative
error required to remain within 20\,\%, A2 predicts
...
error $-0.9661$): both predictions fall short of the published
figures by more than an order of magnitude. These are blind predictions, not
fits: both fail the acceptance test at every $T_{\mathrm{guard}}$
from 10 to 100\,$\mu$s, with
...
slots [24.459, 24.769] for Wireless Paxos, outside [500, 726]---again
mismatches, at the slot-count level this time. All three profiles remain
regression tests so that the frozen calibration cannot silently drift.

These three mismatches are failures of the blind prediction, not
validations of the model. The simulator's wall-clock latency output is
therefore not licensed for comparison against published figures. The
reason remains unresolved: the slot-duration model used to convert slot
counts into milliseconds yields a lower bound on a physically realisable
slot; the published firmware slot lengths disagree with that model by
roughly an order of magnitude; the gap is unexplained; no calibration of
either against the other has been performed; and the paper accordingly
makes no wall-clock latency claim. The 475\,ms and 289\,ms figures remain
unverified published targets with open citation debt.

\subsubsection{Layer 3: sensitivity and invariance}
The third layer evaluates robustness to assumptions represented by the
simulator. We compare the two configured flood-retransmission settings,
$\Nretx \in \{0,1\}$, and evaluate the protocols across topologies spanning
diameters from 1 to 26. Physical slot duration is treated analytically:
because no wall-clock duration enters the simulator's transition relation,
changing $\Tslot$ leaves all slot counts, node-slot counts, duty-cycle
ratios, and CI/CE ratios unchanged, and only rescales quantities subsequently
expressed in milliseconds or decisions per second. It is therefore a unit
conversion rather than an experimental sensitivity axis. Reception losses
are drawn independently for each link and attempt. The evaluation therefore
supports conclusions under this i.i.d. loss model; temporally correlated and
bursty losses are outside its scope. What matters for a comparative claim is
not that the numbers are reproduced exactly under every assumption, but that
the sign of the difference is stable; we report the range over which it
holds. The topology sweep of Section~\ref{sec:results} is itself an
invariance test of this kind. The line topology is the regime in which the
sign does not hold: there, a pipelined protocol is not competitive.
...
Unknown &
Not modelled; conclusions are limited to i.i.d. loss \\
\addlinespace
Node crash failures &
Neither &
...
As Fig.~\ref{fig:thr-nodes} shows, throughput falls with $\Nnodes$ for
every protocol, since the diameter and hence the length of each flood
round grows, and more nodes must be reached before all-node completion
holds. The slopes differ, however, and
...
\subsubsection{Scaling of CE round length}
...
[0.115956905, 0.132213550], AIC 20.403476 and $R^2=0.995552$. The
pre-registered rule identifies $\alpha \sim 0$ with Hypothesis A and
$\alpha \sim 1$ with Hypothesis B; a CI excluding both yields
\texttt{SUBLINEAR-INTERMEDIATE}. Both reported CIs exclude 0 and 1, so both
arms receive that classification. Historical width-4 windows produced
residual ratios 3.5234x at $\Nnodes=180$ for 2PC and 3.7776x at
$\Nnodes=188$ for Paxos. At $\Nnodes=180$, the historical 2PC residual
3.523 agrees closely with the new sweep constant gap 3.508, a difference of
-0.015. The Paxos anchor at $\Nnodes=188$ lies outside this sweep's grid and
is therefore not compared under the uniform $W=10$ procedure.
...
Fig.~\ref{fig:thr-loss} shows that raising the loss rate from 0 to
20\,\% degrades most protocols gently.
TOM on both substrates and Paxos over \CI{} lose 14.33\,\%, 18.80\,\%, and
14.35\,\% of their throughput, respectively, across that range and their curves remain
...
perfectly flat at a low level, declining by 4.65\,\% and 6.29\,\%, respectively, over the whole
...
This matters for the interpretation of Fig.~\ref{fig:thr-loss}. While some
protocols degrade gracefully, 2PC over \CI{} suffers a severe drop in
reliability: its commit rate falls to 70.07\,\% at 20\,\% loss.
The 39.35\,\% throughput reduction is therefore driven substantially by the mass
loss of decisions, not just a longer workload completion time. This locates the fragile point
...
\emph{single} decision. On the logarithmic vertical axis of
Fig.~\ref{fig:lat-nodes}, latency growth with $\Nnodes$ is close to
multiplicative for the vote-based protocols and almost flat for TOM.
...
range of $\Nnodes$ and separate only at $\Nnodes = 188$ (about 6.8
against 8.1 slots, this time in favour of \CI). This coincidence is
...
especially 2PC. Latency for 2PC over \CI{} rises from 16.2 slots at
$\Nnodes = 6$ to 1200.8 slots at $\Nnodes = 188$, and Paxos over
\CI{} from 8.1 to 639.2 slots. \textbf{This does not contradict
...
The logarithmic view in Fig.~\ref{fig:progress-log} reveals a clear and,
at first sight, unexpected ordering. One might attribute early agility to
the goal-oriented \CE{} physical layer, since a \CE{} round terminates as
soon as its goal is met.
...
The linear view in Fig.~\ref{fig:progress-lin} presents an almost
inverted picture and shows that initial agility is a poor predictor of
workload completion time. Once warm-up
...
latency in the entire system at 126.8 slots in the scalability baseline---completes the workload
...
directly and quantitatively by computing the effective pipeline depth from
Little's law, Equation~\eqref{eq:depth}, as the mean over the fifteen seeds
of the per-seed product of throughput and latency. The result is
Table~\ref{tab:depth}.

\begin{table}[!t]
\caption{Effective pipeline depth. Throughput and latency columns are the
means over the fifteen seeds; depth is computed from Little's law
(Equation~\eqref{eq:depth}) as the mean over the fifteen seeds of the
per-seed product of throughput and latency, so it is not the product of the
two printed columns.}
\label{tab:depth}
\centering
\footnotesize
...
\subsection{Hardware Sanity Check: Pipelined versus Sequential Execution}
\label{sec:results-hardware}

The slot-level simulation predicts that pipelining should primarily improve
the rate at which a workload completes, while leaving the lifetime of an
individual consensus largely unchanged. We tested this qualitative
prediction in an 8-node BlueFlood hardware experiment using the same
workload of 200 committed messages in both modes. The experiment compared
a sequential, non-pipelined configuration with the proposed pipelined
configuration.

\begin{table*}[!t]
\caption{Hardware comparison for a workload of 200 committed messages.
Throughput is reported in decisions per local communication slot and
therefore measures slot-normalised protocol efficiency rather than
wall-clock throughput.}
\label{tab:hardware-pipeline}
\centering
\footnotesize
...
\textbf{Non-pipeline} &
\textbf{Pipeline, fill} &
\textbf{Pipeline, steady state} \\
\midrule
Committed messages             & 200      & 200      & 200      \\
Communication rounds            & 1626     & 207      & 200      \\
Local communication slots       & 6504     & 828      & 800      \\
Slot-normalised throughput      & 0.030750 & 0.241545 & 0.250000 \\
Mean consensus latency (ms)     & 1345     & 1338.890 & 1338.890 \\
\bottomrule
\end{tabular}
\end{table*}

The hardware result confirms the qualitative prediction of the simulation.
In the first 200-commit window, pipelining reduced the number of
communication rounds and local communication slots by 87.27\%. After the
pipeline had filled, the system produced one committed decision per round,
requiring 800 slots for 200 decisions. The mean lifetime of an individual
consensus, however, remained approximately eight rounds and 29 slots in
both pipelined windows. Thus, the principal benefit came from overlapping
independent consensus instances rather than from shortening the lifetime
of one instance.

The slot-normalised throughput increased by a factor of
$0.241545/0.030750 = 7.86$ in the initial pipelined window and by a factor
of $0.250000/0.030750 = 8.13$ after pipeline fill. These ratios should not
be interpreted as an eight-fold increase in wall-clock throughput: their
denominator is the number of local communication slots. Using the elapsed
experiment time reported by the hardware logs, the first pipelined window
completed 200 decisions in 39.392\,s, compared with 42.308\,s for the
non-pipelined run, corresponding to a 1.07-fold or 7.4\% increase in
wall-clock throughput. The steady-state pipelined window completed in
38.241\,s, corresponding to a 1.11-fold or 10.6\% increase relative to
the non-pipelined run.

The direct latency of one consensus changed only slightly, from
approximately 1.345\,s without pipelining to 1.338890\,s with pipelining,
a reduction of about 6.11\,ms or 0.45\%. This separation between nearly
unchanged single-decision latency and substantially lower amortised
communication work is consistent with the central mechanism of the
proposed architecture.

Both configurations completed the compared 200-commit workload without a
failed decision. The pipelined run reported 100\% local RX reliability in
the compared windows, whereas the non-pipelined run reported 96.38\%
local RX reliability and five CRC failures. These reliability figures are
reported as observations from one hardware run and are not used as a
statistical comparison.

Finally, the hardware experiment measured communication activity rather
than electrical energy. The reductions in communication slots, TX slots,
RX slots, and TX airtime are therefore energy proxies, not joule
measurements. The hardware result should consequently be read as an
independent sanity check that the simulated concurrency mechanism appears
on the target platform, rather than as a complete replicated hardware
validation or a direct measurement of electrical energy.
...
Fig.~\ref{fig:energy-mean} shows the two architectures side by side for
each protocol.
...
to $5.67$ under 2PC is not driven by proposal sharing; it is the number of slots
a decision occupies, measured as total simulated slots per committed decision
...
The distributional view of Fig.~\ref{fig:energy-box} adds two further
observations. First, the \CI{} and
...
concurrency rises from 1.00 to 21.25.
...
The latency--energy plane of Fig.~\ref{fig:lat-energy} summarises the
main conceptual result of this evaluation in a single picture. The three
baseline points lie on the reference line of Equation~\eqref{eq:ece}---not
approximately but
...
in Fig.~\ref{fig:eff-nodes} show that their conclusions survive a change
of scale. Efficiency declines
...
Full mesh    & 1  & 26.00 \\
Partial mesh & 3  & 9.30 \\
Random       & 4.20 (4--5)  & 4.40 \\
Scale-free   & 4.13 (4--5)  & 3.78 \\
Line         & 26 & 1.93 \\
\bottomrule
\end{tabular}
\end{table}
...
The density-controlled sweep bounds the density confound across $\Nnodes$:
density error has no systematic trend with $\Nnodes$, while both \CE{} arms
remain \texttt{SUBLINEAR-INTERMEDIATE}. The measurements bound the effect of
density variation, but do not prove an external explanation for the residual;
they establish only that infrastructure quality cannot account for it entirely.
Table~\ref{tab:diameter} summarises the measured throughput change on moving
from the random topology to the full mesh.
...
worse.} The latency measurements in Fig.~\ref{fig:lat-topo} identify the
cause. The latency of Paxos
...
throughput at an almost identical rate of about 72\,\%
(Fig.~\ref{fig:thr-topo}), because the length
...
against 0.220 in the scale-free graph; in no case does the relative difference
exceed 1.239 per cent. The TOM boundary case analysed in
...
\CE{} counterpart for all three protocols on the four dense topologies; on
the line topology this holds for TOM and Paxos, while 2PC is an exception
because near-zero commits render its per-decision energy meaningless. The
...
throughput and Paxos over \CE{} 10.6\,\%, against 26.7\,\% and 10.6\,\% over
...
the mean over fifteen seeds and each change is the ratio of the means.}
\label{tab:degree}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
Paxos (\CI) & 0.1958 & $+2.9$\,\%  & 59.8  & $-2.5$\,\%  \\
2PC (\CI)   & 0.1682 & $+0.1$\,\%  & 124.7 & $-1.6$\,\%  \\
TOM (\CE)   & 0.2170 & $+1.3$\,\%  & 4.6   & $-1.3$\,\%  \\
\addlinespace
Paxos (\CE) & 0.0629 & $-10.6$\,\% & 15.9  & $+11.9$\,\% \\
...
protocols are faster on eight to ten of them.
...
very topology at a full commit rate and a throughput of 0.0611---which
...
decision and the efficiency reported for that cell both result from division
by a denominator close to zero and merely restate the commit rate of
0.2\,\%.
...
First, the main parameter sweep, topology study and generalisation results
are obtained in simulation at the granularity of a slot. The scope of that
abstraction, the direction of the bias it introduces and the calibration
procedure used to control it are set out in
Section~\ref{sec:method-validation}. Section~\ref{sec:results-hardware}
provides a separate 8-node hardware sanity check, but it is a single
workload comparison and does not replace replicated hardware experiments or
a direct measurement of electrical energy. Second, the fault model covers packet
...
pipeline window is fragile; that instability shows up in two
regimes: at low loss and in small networks the commit rate stays close to
complete and the cost appears mainly as a longer run, whereas under heavy
loss the commit rate itself falls (to 70.07\,\% for 2PC over \CI{} at 20\,\%
loss), and at scale or on large-diameter topologies the loss of decisions
dominates, with the commit rate dropping to 23.2\,\% in the largest
scalability network and 0.2\,\% for 2PC on the line topology. The cumulative
progress view then shows that the high latency of \CI{} is misleading: all
three pipelined protocols finish the workload before either vote-based \CE{}
protocol.
