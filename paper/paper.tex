...
\begin{tikzpicture}[font=\footnotesize]
  % --- y1 = sin(2*pi*5.5*t + 90 deg) ---
  \draw[thin] (-0.30,0) -- (7.30,0);
  \node[anchor=east,font=\scriptsize] at (-0.35,0.55) {$y_1$};
  \draw[thin] plot[domain=0:6.8,smooth,samples=90]
    ({\x},{0.50*sin(deg(2*pi*5.5*\x/6.8)+90)});
  % --- y2 = sin(2*pi*7.6*t - 90 deg) ---
  \node[anchor=east,font=\scriptsize] at (-0.35,-1.25) {$y_2$};
  \draw[thin] plot[domain=0:6.8,smooth,samples=90]
    ({\x},{-1.80+0.50*sin(deg(2*pi*7.6*\x/6.8)-90)});
  % --- sum and its envelope ---
  \node[anchor=east,font=\scriptsize] at (-0.35,-3.10) {$y_1{+}y_2$};
  \draw[thin,dashed] plot[domain=0:6.8,smooth,samples=120]
    ({\x},{-3.65+1.00*abs(sin(deg(pi*(5.5-7.6)*\x/6.8/2)))});
  \draw[thin,dashed] plot[domain=0:6.8,smooth,samples=120]
    ({\x},{-3.65-1.00*abs(sin(deg(pi*(5.5-7.6)*\x/6.8/2)))});
  \draw[thick] plot[domain=0:6.8,smooth,samples=180]
    ({\x},{-3.65+0.50*sin(deg(2*pi*5.5*\x/6.8)+90)
              +0.50*sin(deg(2*pi*7.6*\x/6.8)-90)});
  % --- time axis ---
  \draw[->] (-0.30,-4.55) -- (7.40,-4.55);
  \node[anchor=north,font=\scriptsize] at (3.55,-4.60) {time};
\end{tikzpicture}}
\caption{Two concurrent sinusoids of different frequency and their sum.
The sum is amplitude-modulated by an envelope whose period is set by the
frequency difference; a receiver that tracks the envelope rather than the
carriers sees one slow signal instead of two fast ones. Adapted from
\cite{ferrari11glossy}.}
\label{fig:ct-beating}
\end{figure}

The physical-layer effect is easiest to see before the protocol detail. Fig.~\ref{fig:ct-beating} shows two concurrent sinusoids of different frequency arriving at one receiver: their sum is amplitude-modulated by a slow envelope whose period is set by the frequency difference, so a receiver that tracks the envelope recovers one signal from two overlapping transmissions.
...
\resizebox{3.4in}{!}{%
\begin{tikzpicture}[font=\footnotesize]
  \node[anchor=south,font=\scriptsize] at (1.55,1.85) {\textbf{time offset}};
  \node[anchor=south,font=\scriptsize] at (1.55,0.55) {transmitter $A$};
  \draw[thin] (0.00,0.00) -- (3.10,0.00);
  \draw[thin] (0.00,1.30) -- (3.10,1.30);
  \draw[thick] (0.55,0.00) -- (0.55,1.30);
  \draw[thick] (1.65,0.00) -- (1.65,1.30);
  \node[font=\scriptsize] at (1.10,0.62) {$t_1$};
  \draw[thin,dashed,->] (2.45,0.65) -- (1.80,0.65);
  \node[font=\scriptsize,anchor=west] at (2.45,0.65) {capture zone};

  \node[anchor=south,font=\scriptsize] at (1.55,-1.55) {transmitter $B$};
  \draw[thin] (0.00,-2.85) -- (3.10,-2.85);
  \draw[thin] (0.00,-1.55) -- (3.10,-1.55);
  \draw[thick] (1.35,-2.85) -- (1.35,-1.55);
  \draw[thick] (2.55,-2.85) -- (2.55,-1.55);
  \node[font=\scriptsize] at (1.95,-2.23) {$t_2$};
  \draw[thin,dashed,->] (2.45,-2.20) -- (2.00,-2.20);
  \node[font=\scriptsize,anchor=west] at (2.45,-2.20) {capture zone};

  \node[anchor=south,font=\scriptsize] at (5.65,1.85) {\textbf{phase offset}};
  \fill[black!12] (4.20,0.00) rectangle (7.10,1.30);
  \draw[thin] (4.20,0.00) rectangle (7.10,1.30);
  \draw[thick] (4.80,0.00) -- (4.80,1.30);
  \draw[thick] (5.45,0.00) -- (5.45,1.30);
  \draw[thick] (6.70,0.00) -- (6.70,1.30);
  \node[font=\scriptsize] at (5.12,0.62) {$A$};
  \node[font=\scriptsize] at (6.07,0.62) {$B$};
  \draw[thin,dashed] (4.20,0.65) -- (7.10,0.65);

  \node[anchor=south,font=\scriptsize] at (5.65,-1.55) {received power};
  \draw[thin] (4.20,-2.85) -- (7.10,-2.85);
  \draw[thin] (4.20,-1.55) -- (7.10,-1.55);
  \draw[thin,->] (4.20,-2.20) -- (5.30,-2.20);
  \node[font=\scriptsize,anchor=west] at (5.30,-2.20) {$A$ decoded};
  \draw[thin,->] (5.45,-2.20) -- (6.60,-2.20);
  \node[font=\scriptsize,anchor=west] at (6.60,-2.20) {$B$ decoded};
\end{tikzpicture}}
\caption{Schematic of the capture effect. The receiver locks onto whichever
of two overlapping transmissions arrives with sufficient power margin; the
diagram marks the offset windows in which one transmitter is decoded and
the other is suppressed. It is a conceptual diagram, not a reproduction of
any measurement. Adapted from \cite{wilhelm14reception}.}
\label{fig:ct-capture}
\end{figure}

Fig.~\ref{fig:ct-capture} shows the complementary case. When the two overlapping transmissions are not identical, the receiver locks onto whichever arrives with a sufficient power margin and suppresses the other; the diagram marks the offset windows in which each transmitter is decoded.
