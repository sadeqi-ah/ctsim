...
round trip to close. Fig.~\ref{fig:pipeline} sketches that overlap on a
...
\begin{tikzpicture}[
  font=\scriptsize,
  fld/.style={draw, minimum height=0.95cm, inner sep=2pt, align=center},
  br/.style={decorate, decoration={brace, amplitude=4pt}},
]
\node[fld, minimum width=1.35cm] (f1) at (0,0) {term\\ident.};
\node[fld, minimum width=1.45cm, right=0pt of f1] (f2)
  {last acc.\\term};
\node[fld, minimum width=1.95cm, right=0pt of f2] (f3)
  {particip.\ bitmap\\($\Nnodes$ bits)};
\node[fld, minimum width=1.00cm, right=0pt of f3] (f4) {NACK};
\node[fld, minimum width=1.75cm, right=0pt of f4, fill=black!8] (f5)
  {pipelined\\payload};
\draw[br]
  ($(f1.north west)+(0,2pt)$) -- ($(f4.north east)+(0,2pt)$)
  node[midway, above=4pt] {control header};
\draw[br]
  ($(f5.north west)+(0,2pt)$) -- ($(f5.north east)+(0,2pt)$)
  node[midway, above=4pt] {payload};
\end{tikzpicture}}
...
Fig.~\ref{fig:frame} shows the field layout.
...
node. The sender writes the bits it currently holds for the instance named
in the header; a node that receives or merely overhears the frame merges
those bits into its own record of that instance and of every earlier one,
and contributes its own bit when its turn to flood arrives. Because a
constructive-interference flood requires every retransmission to carry a
byte-identical payload, a relay cannot modify the frame in flight: the
bitmap therefore accumulates in the replicated local state of the nodes
rather than inside a single travelling frame, and it still costs no
...
overhearing. Every node that receives the frame sets its own bit for the
current instance and floods that bit when its own turn arrives, so votes
accumulate as the schedule advances through the remaining nodes of the
cycle. A vote is cumulative: a bit set for the term named in the header
also counts for every earlier term still in flight, so a single bitmap
advances several in-flight decisions at once and no instance has to be
suspended while another collects its votes.
