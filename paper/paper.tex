...
\cite{du17pando}. A recent preprint applies constructive interference to
scalable, energy-efficient predictive data collection in wireless sensor
networks \cite{muldoon25stair}; like the systems above, it moves data
rather than decisions, carrying no protocol logic in its pipeline---no
votes, no quorums, no ordering constraint---so the problem this paper
addresses does not arise for it. Both target one-way transfer of a
large, passive payload. Neither carries any protocol logic: there are
no votes to
