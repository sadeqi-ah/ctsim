...
directly and quantitatively from Little's law, Equation~\eqref{eq:depth}:
the throughput and latency columns are the means over the fifteen seeds,
while the depth column is the mean over the fifteen seeds of the per-seed
product of throughput and latency, so it is not the product of the two
printed columns. The result is Table~\ref{tab:depth}.

\begin{table}[!t]
\caption{Effective pipeline depth. Throughput and latency columns are the
means over the fifteen seeds; depth is the mean over the fifteen seeds of
the per-seed product of throughput and latency, so it is not the product of
