...
> EXECUTED 2026-09-17 at `main` =
> `8a3d347dd06cacbe323f2a94d01a569fbfb525b1`.  Every prediction in this
> plan has now been checked against measurement; see the STEP 2.49
> ANNOTATION blocks below.  Headline result: the staleness hypothesis
> behind the rerun (that a quorum fix necessarily changes derived data)
> was empirically refuted -- the full pipeline ran clean end to end and
> every tracked file came back byte-identical (`git diff --stat` empty).
...
expected.
STEP 2.49 ANNOTATION: the at-risk counts above (158 of 300
scalability, 31 of 75 topology `paxos_pipeline,ci` rows with
`piggybacks > 0`) were themselves overtaken by the owner run of
2026-09-17 at `8a3d347d`: the full sweep rerun plus
`regen_figures.py --force` produced an empty `git diff --stat` --
no committed row changed at all, byte-identical or otherwise.
The at-risk/changed framing survives as history, not as a
live prediction.
...
STEP 2.49 ANNOTATION: refuted as unknown -- measured 2 min 11 s
for all 1800 experiments (owner run, 2026-09-17, at
`8a3d347d`).  Original text above retained.
...
STEP 2.49 ANNOTATION: refuted as unknown -- measured 5 s for all
450 experiments (owner run, 2026-09-17, at `8a3d347d`).  Original
text above retained.
...
STEP 2.49 ANNOTATION: the whole table has been decided by the owner
run of 2026-09-17 at `8a3d347d`.  Outcome, row by row, with the
original predictions above retained unchanged: `per_decision_energy.csv`
-- predicted "YES, changes", REFUTED: the file regenerated
byte-identical, hash `7386c9467760b7c4bacb50d30705dfc7f00b0e8f` again
(same for the other three pinned sources: `distribution_stats.csv`
`ace5a6b0e265259991efde61df3f8c65805543a0`, `distribution_stats.md`
`73a554b1daf40c8206de2fd85256b9592ffba345`,
`integer_multiple_check.md`
`7c9cfbe5921a3d082fbfa3ae254d7110164032def`); the three
"UNKNOWN - REQUIRES A RUN" rows are resolved as NO CHANGE -- no hash
update needed, `tests/validation.rs` untouched.
...
Wall-clock cost: UNKNOWN - REQUIRES A RUN
STEP 2.49 ANNOTATION: refuted as unknown -- Step 9 itself was
executed by the owner on 2026-09-17 at `8a3d347d`; the four pinned
source blobs regenerated byte-identical, so there was no wall-clock
cost to attribute to hash churn and no hash update round was needed.
Original text above retained.
...
STEP 2.49 ANNOTATION: refuted -- the owner run of 2026-09-17 at
`8a3d347d` executed Step 9 and `cargo test --all` passed with 35
tests green (validation suite 22 passed, 51.86 s); no `assert_blob`
failure occurred because the four pinned blobs regenerated
byte-identical.  The prediction is recorded above, unchanged.
...
- The Step 9 prediction is UNTESTED, not superseded.  Step 9
  ...
  Step 9.
  STEP 2.49 ANNOTATION: the UNTESTED status above is now resolved --
  Step 9 WAS executed by the owner on 2026-09-17 at `8a3d347d`
  (outside any git commit, so the `--grep='regenerate'` evidence
  above remains literally true of committed history), and the four
  pinned blobs regenerated byte-identical.  The prediction's
  subject has now been tested; its outcome (expected hash churn)
  did not occur.
...
STEP 2.49 ANNOTATION: refuted as unknown -- measured on the owner
run of 2026-09-17 at `8a3d347d`: scalability sweep (1800
experiments) 2 min 11 s; topology sweep (450 experiments) 5 s.
Original text above retained.
...
Lines 1850, 2281: `sources/per_decision_energy.csv`
  ...
  STEP 2.49 ANNOTATION: REFUTED -- the owner run of 2026-09-17 at
  `8a3d347d` regenerated this file byte-identical
  (`7386c9467760b7c4bacb50d30705dfc7f00b0e8f`); no hash update was
  needed.  Original prediction retained above.

Lines 1854, 2285: `sources/distribution_stats.csv`
  SHA1 `ace5a6b0...` -- UNKNOWN - REQUIRES A RUN
  STEP 2.49 ANNOTATION: resolved -- regenerated byte-identical
  (`ace5a6b0e265259991efde61df3f8c65805543a0`), owner run
  2026-09-17 at `8a3d347d`; no hash update needed.

Lines 1858, 2289: `sources/distribution_stats.md`
  SHA1 `73a554b1...` -- UNKNOWN - REQUIRES A RUN
  STEP 2.49 ANNOTATION: resolved -- regenerated byte-identical
  (`73a554b1daf40c8206de2fd85256b9592ffba345`), owner run
  2026-09-17 at `8a3d347d`; no hash update needed.

Lines 1862, 2293: `sources/integer_multiple_check.md`
  SHA1 `7c9cfbe5...` -- UNKNOWN - REQUIRES A RUN
  STEP 2.49 ANNOTATION: resolved -- regenerated byte-identical
  (`7c9cfbe5921a3d082fbfa3ae254d7110164032def`), owner run
  2026-09-17 at `8a3d347d`; no hash update needed.
