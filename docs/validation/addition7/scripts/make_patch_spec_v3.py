...
per-decision energy efficiency than CE-based counterparts. We""",
    ...
    "5.671896 +- 0.273692 (t-interval on the 15 per-seed ratios, decision "
    "123). 'amortized' deleted: decision 36 "
...
2PC (\\CI)   & 0.1681 & 126.8 & 21.25 \\\\""",
    ...
    "21.254664 +- 0.218137 (t-interval, t(0.975,14) = 2.144787; v2 mislabelled "
    "this one as jackknife). Latency from sweep_summary.csv. "
...
reported in this paper are cumulative, so no result depends on that gap.""",
    "The gap, 15 seeds: 1 - sum(decisions.csv amortised)/sum(runs.csv "
    "awake_tick) -> 0.00084857 / 0.01662420 / 0.04571172, printing 0.08 / "
    "1.66 / 4.57 %. NOTE, v3: the runs.csv awake_outside_windows share is a "
    "different quantity, 0.00084857 / 0.01662420 / 0.03299890 -- it coincides "
    "for TOM and Paxos but not for 2PC, so the 2PC figure must be read off "
    "the per-decision file and not off awake_outside_windows. The printed "
    "triple is unchanged; only this attribution is corrected. "
    "The 4500 CE decisions: decisions.csv, amortised "
...
$5.67 \\pm 0.27$ for 2PC (95\\,\\% intervals over fifteen seeds).""",
    "runs.csv cumulative_per_dec, CE over CI, 15 seeds. Point estimate is the "
    "ratio of means: 3.827738 and 5.671896. DECISION 123 changes the interval: "
    "the printed 0.08 and 0.30 were jackknife half-widths of the ratio of "
    "means (0.075337 and 0.298615); the reported interval is now the "
    "t-interval on the fifteen per-seed ratios at t(0.975,14) = 2.144787, "
    "half-widths 0.067094 and 0.273692, so 0.08 -> 0.07 and 0.30 -> 0.27. "
    "Centred on the ratio of means the intervals are [3.760644, 3.894832] and "
    "[5.398204, 5.945588]; the mean of the per-seed ratios (3.832605 and "
    "5.704037) differs from the ratio of means by 0.005 and 0.032, well "
    "inside the half-width, so the choice of centre does not move a printed "
    "digit. Neither printed digit is resolved by its own interval; the "
    "interval is printed beside the value for that reason.",
    "36, 42, 43, 51")
...
$6.17$ under \\CI{} against $4.68$ to $24.34$ under \\CE.""",
    ...
    "print 1.6 %. Slots per decision = simulated_slots/committed, ratio of "
    "sums: CI 4.703333/5.354000/6.170485, CE 4.676000/14.207333/24.335333. "
    "DECISION 138: the printed upper end is 6.17, the ratio of sums, not the "
    "6.18 of the mean of per-seed ratios (6.178465); the ratio of sums is the "
    "aggregation used for every other ratio in the paper, and the definition "
    "is now written into the sentence. 6.18 occurs nowhere in the base text, "
    "only in the v2 replacement. DELETED: 'it tracks the depth column of "
...
# ---------------------------------------------------------------- 1904-1907
# NEW IN v3. One of the only two amorti[sz] sites in the base where the
# retired metric carries an argument rather than naming the decomposition.
add(1904, 1907,
...
ceiling does not depend on which of the two is used.""",
    "No new numbers. Decisions 36 and 44 retire the amortised decomposition as "
    "the basis for every printed ratio, so the paragraph that establishes "
    "E_CE = N x L must not rest on that metric alone. DELETES the reference to "
    "Equation~eqref{eq:amortized} at base 1904 and states the collapse for "
    "both definitions, which is what the data show: decisions.csv gives "
    "amortised = 27 x latency exactly on all 4500 CE rows, and the two "
    "accounts agree on every CE row of runs.csv, so the ceiling is "
    "definition-independent under CE. SMALLEST-SPAN NOTE, decision 55: base "
    "1903 ends with 'Substituting both conditions into', outside the span, so "
    "the replacement opens with the grammatical complement of 'into'. Base "
    "1906 is RE-SUPPLIED VERBATIM and not changed. eq:amortized keeps its "
    "other references (base 1936, 1937 and the 2190-2197 replacement), so no "
    "label is orphaned and no equation is deleted.",
    "36, 44")
...
ninety-sevenfold range of energy per decision.""",
    "Two files, not one: the sleep counter is exactly zero in all 1125 CE "
    "rows, being 225 rows in plots/topology/results/sweep_summary.csv plus "
    "900 in plots/scalability/results/sweep_summary.csv. Diameters present "
    "are 1, 3, 4, 5 and 26 across the five topologies. CE energy per "
    "decision, (listen+flood+sleep)/committed as group means over the 15 "
    "protocol-topology groups, spans 73.93 (TOM, full_mesh) to 7190.93 (2PC, "
    "line) node-slots, a factor of 97.3; the same aggregation reproduces "
    "tab:baseline exactly at the reference point (126.25 / 383.60 / 657.05), "
    "and 7190.93 is the value the deleted tab:identity carried at base 2335. "
    "CORRECTS v2, which printed 'forty-fold' from a range of 126.3 to 5136.5 "
    "that no aggregation of the committed CSVs reproduces. Deletes the "
    "reference to tab:identity, which the following entry removes.",
    "32")
...
slot shared between $k$ decisions contributes $1/k$ to each.""",
    ...
    "multiples of 27. CORRECTS v2, which claimed the CI values 'are not "
    "integers at all': 82 of 1500 Paxos-CI and 20 of 1484 2PC-CI values are "
    "in fact integers, so the sentence now reads 'in general not integers'. "
    "The arm-level claim survives unchanged, because no CI arm has every "
    "value an integer multiple of 27: TOM-CI is integral throughout but only "
    "81 of its 1500 values are multiples of 27, and no Paxos-CI or 2PC-CI "
    "value is one at all. DELETES the fifteen-row table float: "
...
line topology, where two of the fifteen seeds commit 99 proposals of 100.""",
    ...
    "%, so '22 and 31' stands, and the shortfall is exactly 1 - duty: the "
    "runs.csv duty cycle at the random point gives 30.71/30.67/30.54 %, "
    "matching the sweep to the digit. Residual: res = 1/d - 1 with d = "
    "(committed/end_slot) x avg_latency, computed per seed on the topology "
    "sweep. It is zero to 2.2e-16 in 14 of the 15 protocol-topology groups; "
    "the sole nonzero group is line/2PC-CE, group mean 0.25765264 % (mean of "
    "per-seed residuals), worst single seed 37 at 1.93620964 % and seed 23 at "
    "1.92857998 %, 13 of 15 seeds committing 100. The printed 1.94 % is the "
    "WORST SEED and is named as such; the group mean 0.26 % is not printed. "
    "Formula is 1/d - 1, not 'the reciprocal of the depth'. This is NOT the "
    "orphan share f = orphan/(end_slot + orphan) recorded under decision 114 "
    "(1.890834 / 1.898617 %); the two are related by res = f/(1-f) per "
    "decision 113, and the sentence here describes res, so 1.94 % stands.",
    "10, 32, 42, 49")
...
# ---------------------------------------------------------------- 2376-2377
# NEW IN v3. The second of the two argument-carrying amorti[sz] sites: the
# subsection measures depth, so its conclusion must be stated about depth.
add(2376, 2377,
    """This invariance shows that the pipeline depth is an
\\emph{architectural design parameter} and not a network accident. The""",
    "No new numbers. Decision 36 retires the amortisation factor, so the "
    "conclusion of the subsection must be stated about the quantity the "
    "subsection actually measures. The preceding entry at 2371-2374 prints "
    "depth, and sweep_summary.csv gives depth = (committed/end_slot) x "
    "avg_latency invariant across the four dense topologies: TOM "
    "1.000/1.000/1.000/1.002, Paxos 11.589/11.623/11.662/11.657, 2PC "
    "21.446/21.455/21.255/20.954. SMALLEST-SPAN NOTE, decision 55: the "
    "changed words end mid-line at base 2377, so the remainder of that line "
    "is RE-SUPPLIED VERBATIM and not changed -- 'and not a network accident. "
    "The', whose continuation at base 2378 ('pipeline structurally holds a "
    "certain number of proposals in flight') lies outside the span. Blank "
    "base line 2375 separates this entry from 2371-2374, so no blank base "
    "line falls inside either span.",
    "36, 42")
...
dense topologies, so the mechanism that produces the energy saving is""",
    ...
    "1/depth - 1. 1125 CE rows with sleep = 0 throughout, being 225 topology "
    "rows plus 900 scalability rows across two sweep_summary.csv files. The "
...
B.append({
    "where": "1470-1478, base 2505-2509, and the surrounding commit-rate prose",
    "what": "the 98.9 % commit rate discussion, the zero-commit rows of the "
            "topology and scalability tables, and the '(190,180)' efficiency "
            "figure at base 2507",
    "why": "Decision 12's artefact-level treatment and decision 10's "
           ...
           "than 100, all 2PC over CI, one of them zero at N = 188. The 2507 "
           "figure is not recoverable from the committed data under decision "
           "10: at line/2PC-CI, N = 27, 5 % loss, exactly one of the fifteen "
           "seeds commits at all (seed 29, 3 decisions, a 0.2 % commit rate, "
           "81940.0 slots per decision), so a conditional mean would rest on "
           "n = 1 and cannot honestly be reported as a mean.",
})
...
assert len(E) == 34, "entry count changed -- update this number deliberately"
